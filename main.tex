...
\noindent\textbf{Note on Training Configuration:}

Due to local computational resource limitations (M1 Mac with 8GB RAM), all models in this project were trained with relatively few epochs. This constraint resulted in:
\begin{itemize}
    \item Limited training epochs (5-8 total across phases)
    \item Potentially lower absolute accuracy values, especially for quantized variants
    \item Trade-off between training time and model convergence
\end{itemize}

However, this limitation does \textbf{not affect the validity of the analysis} because:
\begin{itemize}
    \item Relative performance comparisons remain consistent across all model variants
    \item Hardware-aware optimization trends (latency, memory, energy) are independent of training epochs
    \item The focus is on optimization methodology and cross-platform scalability rather than achieving state-of-the-art accuracy
    \item All models were trained under identical conditions, ensuring fair comparison
\end{itemize}
...
\includegraphics[width=0.8\textwidth]{charts/int8_quantization_comparison_energy.png}

\begin{itemize}
    \item Method: TFLite converter with representative dataset (2000 samples)
    \item Result: \textbf{DEGRADED} - 31-54\% accuracy (15-34\% drop from baseline)
    \item Cause: MobileNetV2's depthwise separable convolutions sensitive to uniform INT8
\end{itemize}

\subsubsection{Quantization-Aware Training (QAT) INT8}
\begin{itemize}
    \item Method: tf.keras quantize\_model with QAT
    \item Result: \textbf{IMPROVED} - 35-57\% accuracy ($\sim$3\% better than PTQ)
    \item Conclusion: QAT helps but significant degradation remains; INT8 not recommended for MobileNetV2
\end{itemize}
...
Latency + PTQ INT8 & 31.40\% & 0.938 & [$\triangle$] Degraded \\
Latency + QAT INT8 & 34.80\% & 0.938 & [$\triangle$] Degraded \\
Energy + Dynamic Range & 70.40\% & 1.52 & [$\checkmark$] \textbf{Best Balance} \\
Energy + Mixed Precision & 70.10\% & 2.64 & [$\checkmark$] Good \\
Energy + PTQ INT8 & 53.60\% & 1.676 & [$\triangle$] Degraded \\
Energy + QAT INT8 & 57.10\% & 1.676 & [$\triangle$] Degraded \\
Memory Pruned + Dynamic Range & 51.40\% & 0.53 & [$\checkmark$] Most Compact \\
Memory Pruned + Mixed Precision & 53.10\% & 0.82 & [$\checkmark$] Good \\
Memory Pruned + PTQ INT8 & 35.95\% & 0.605 & [$\triangle$] Degraded \\
Memory Pruned + QAT INT8 & 39.20\% & 0.605 & [$\triangle$] Degraded \\
\hline
\end{tabular}
...
\begin{itemize}
    ...
    \item INT8: Would achieve 306\% efficiency (4× vectorization) but accuracy degradation (15-34\% drop) limits practical use
...
\begin{itemize}
    \item \textbf{INT8 Quantization + Depthwise Convolutions}: Uniform quantization caused significant degradation (15-34\% accuracy drop)
...
\begin{itemize}
    ...
    \item INT8 (theoretical 4× vectorization): Achievable but with significant accuracy trade-offs (15-34\% drop)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Impact}: INT8 becomes preferred for NPU acceleration
    \item \textbf{Design Change}: Need to address INT8 accuracy degradation for MobileNetV2 (current 15-34\% drop)
    \item \textbf{Solution}: Explore alternative architectures (EfficientNet, MobileViT) with better INT8 compatibility, or use QAT techniques
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Quantization}: Dynamic Range recommended; INT8 shows significant accuracy degradation (PTQ: 31-54\%, QAT: 35-57\%)
...
\textbf{Technical}:
\begin{itemize}
    \item MobileNetV2's depthwise separable convolutions experience significant accuracy degradation with INT8 quantization (15-34\% drop)
...
\begin{enumerate}
    ...
    \item \textbf{Avoid} PTQ/QAT INT8 for MobileNetV2 unless extreme compression required (expect 15-34\% accuracy drop)
\end{enumerate}

\textbf{For Future Work}:
\begin{enumerate}
    \item Explore knowledge distillation and advanced QAT techniques to improve INT8 accuracy
    \item Try alternative architectures (EfficientNet, MobileViT) for better INT8 compatibility
